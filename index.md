# autotest [![](reference/figures/autotest.png)](https://docs.ropensci.org/autotest/)
...
is to return nothing (or strictly, `NULL`), indicating that all tests
passed successfully. See the [main package
...
all documented within a single `.Rd` help file. Typing
[`?var`](https://rdrr.io/r/stats/cor.html) shows that the help topic is
`cor`, and that the examples include the three functions, `var`, `cor`,
and `cov`. That result details the 21 tests which would be applied to
the `var` function from the `stats` package. These 21 tests yield the
following results when actually applied:
...
`autotest`-ing a package. The
[`autotest_types()`](https://docs.ropensci.org/autotest/reference/autotest_types.md)
function, used above to extract information on all types of tests, also
accepts a single argument listing the `test_name` entries of any tests
which are to be switched off.
...
Those tests are still returned from
[`autotest_package()`](https://docs.ropensci.org/autotest/reference/autotest_package.md),
but with `test = FALSE` to indicate they were not run, and a `type` of
“no_test” rather than the previous “diagnostic”.
